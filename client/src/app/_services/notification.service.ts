...
export class NotificationService {
  private baseUrl = 'https://localhost:5000/api/notifications';
  ...
  loadUnread(userId: number): void {
    this.http.get<Notification[]>(`${this.baseUrl}/unread/${userId}`)
      .subscribe(notifs => {
        if (notifs.length > 0) {
          // Show each in toast
       //   notifs.forEach(n => this.toastr.info(n.message, 'Notification'));
        }
        // Update state
  ...
  markAllAsRead(userId: number): void {
    this.http.post<void>(`${this.baseUrl}/mark-all-read/${userId}`, {})
      .pipe(
        tap(() => {
          const updated = this._notifications.value.map(n => ({
            ...n,
